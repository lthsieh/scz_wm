...
% Obtain ERSP values for individual participants for stats
% 
...
for iInputType = 1:length(inputDataTypes) % baseline or no baseline
    ...
    if isempty(noMeds)
        outPath   = [groupDir, outName, num2str(length(controls)), 'CN_', num2str(length(patients)), 'SZ_', analysisDate, '_stats_fh.mat'];
    else
        outPath   = [groupDir, outName, num2str(length(controls)), 'CN_', num2str(length(patients)), 'SZmed_', num2str(length(noMeds)), 'SZunmed_', analysisDate, '_stats_fh.mat'];
    end
    ...
    for jSub = 1:length(allSubs) % Loop through all subjects
        ...
        if ~exist('precombined', 'var') % Obtain values from first subject (should be same for all)
            if ~isempty(noMeds) % If unmedicated patients processed separately
                precombined.patientsNoMeds = noMeds;
            end
            
            precombined.chanLabels = ERSP.chanLabel;
            precombined.conditions = ERSP.conditions;
            precombined.controls   = controls; % List of controls in analysis
            precombined.patients   = patients; % List of patients in analysis
            precombined.freqs      = finalFreq;
            precombined.times      = finalTime;
        end
        
        clear ERSP % Remove loaded ERSP file to save memory
        
        % Make sure frequencies and times match for each subject
        checkFreqs = intersect(precombined.freqs, finalFreq);
        checkTimes = intersect(precombined.times, finalTime);
        if length(checkFreqs) ~= length(finalFreq) || length(checkFreqs) ~= length(precombined.freqs)
            error('Frequency mismatch %s\n', allSubs{jSub})            
        elseif length(checkTimes) ~= length(finalTime) || length(checkTimes) ~= length(precombined.times)
        ...
        for kCon = 1:length(precombined.conditions) % conditions: Item_hit, order_hit, item_miss, order_miss, order_v_item_hit item_v_order_hit
            for mChan = 1:length(precombined.chanLabels) % electrodes: 64
                
                if subCount == 1 % Must create 'NaN' arrays for each electrode (to remove bad)
                    tempStruc{kCon}.chan{mChan} = nan(length(precombined.freqs), length(precombined.times), totalSubs); % Create NaN arrays to quicken processing
                end
                ...
                if jSub == lastCn       % Control
                    ...
                    precombined.cn.conditions{kCon}.chan{mChan} = (tempStruc{kCon}.chan{mChan}); % Remove 'NaN' values (if bad electrodes exist)
                elseif jSub == lastSz   % Patient
                    if size(tempStruc{kCon}.chan{mChan}, 3) ~= legnth(patients)
                        error('Check Data Dimensions!');
                    end
                    precombined.sz.conditions{kCon}.chan{mChan} = (tempStruc{kCon}.chan{mChan}); % Remove 'NaN' values (if bad electrodes exist)
                ...
                elseif jSub == lastNoMed % Unmedicated patient
                    ...
                    precombined.sznomeds.conditions{kCon}.chan{mChan} = (tempStruc{kCon}.chan{mChan}); % Remove 'NaN' values (if bad electrodes exist)
                    precombined.szsznomeds.conditions{kCon}.chan{mChan} = cat(3,tempStrucSZ{kCon}.chan{mChan},tempStruc{kCon}.chan{mChan});
                end
            end % mChan
        end % kCon
    ...
    fprintf('Saving...\n')
    save(outPath, 'precombined', '-v7.3');
    fprintf('CREATED: %s\n', outPath)
    
    clear precombined tempData % Remove structures to save memory
end % iInputType
